...
type DatabaseInitParameters struct {
	...
	ReadReplicas []ReadReplicasInitParameters `json:"readReplicas,omitempty" tf:"read_replicas,omitempty"`
...
type Database struct {
	...
	// +kubebuilder:validation:XValidation:rule="!('*' in self.managementPolicies || 'Create' in self.managementPolicies || 'Update' in self.managementPolicies) || has(self.forProvider.databaseEngine) || (has(self.initProvider) && has(self.initProvider.databaseEngine))",message="spec.forProvider.databaseEngine is a required parameter"
	// +kubebuilder:validation:XValidation:rule="!('*' in self.managementPolicies || 'Create' in self.managementPolicies || 'Update' in self.managementPolicies) || has(self.forProvider.databaseEngineVersion) || (has(self.initProvider) && has(self.initProvider.databaseEngineVersion))",message="spec.forProvider.databaseEngineVersion is a required parameter"
